...
class ProjectsCategoryIndex extends React.Component {
  ...
  selectTab(e) {
    if (this.state.category !== e.currentTarget.innerText) {
      this.props.fetchIndex(e.currentTarget.innerText).then(this.setState({category: e.currentTarget.innerText}));
    }
  }
  ...
  render() {
    ...
    return (<div className="project-index-container">
      <nav className="category-tags-nav">
        {
          categories.map((category, i) => {
            ...
            return (<button className={klass} onClick={this.selectTab} key={i}>
              {category}
            </button>);
          })
        }
          ...
          <div className="category-projects-list">
            <span className="category-projects-head">NEW & NOTEWORTHY</span>
            {
              this.props.projects.slice(1).map((proj) => {
                return <ProjectsCategoryItem id={proj.id} title={proj.title} image={proj.img_url} funded={Math.floor(proj.pledged_amount / proj.goal * 100)} goToShow={this.goToShow} key={proj.id}/>;
              })
            }
          </div>
        </div>
